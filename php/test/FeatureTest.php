...
class FtCtrl extends SunsetTimesControl
{
...
class FtClient
{
    ...
    private SunsetTimesUtility $utility;

    public function __construct(SunsetTimesUtility $utility, array $options)
    {
    ...
    public function get_utility(): SunsetTimesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SunsetTimesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SunsetTimesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SunsetTimesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SunsetTimesConfig::make_feature($name);
    ...
    private static function buildUrl(SunsetTimesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SunsetTimesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SunsetTimesResponse($fetched);
        }
    ...
    private function fail(SunsetTimesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SunsetTimesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
