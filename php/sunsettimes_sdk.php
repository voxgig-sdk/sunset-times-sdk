...
class SunsetTimesSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = SunsetTimesHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = SunsetTimesHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!SunsetTimesFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, SunsetTimesFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return SunsetTimesUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = SunsetTimesHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = SunsetTimesHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = SunsetTimesHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new SunsetTimesError($op . "_allow",
                "SunsetTimesSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = SunsetTimesHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new SunsetTimesError("graphql_error",
                "SunsetTimesSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function SunriseAndSunset($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_sunrise_and_sunset === null) {
                $this->_sunrise_and_sunset = new SunriseAndSunsetEntity($this, null);
            }
            return $this->_sunrise_and_sunset;
        }
        return new SunriseAndSunsetEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new SunsetTimesSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
