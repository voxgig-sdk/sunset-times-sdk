...
import (
	...
	omni "github.com/voxgig-sdk/sunset-times-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SunsetTimesError); is {
				out := map[string]any{
					"name":    "SunsetTimesError",
...
func omniCtx(arg any, client *sdk.SunsetTimesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SunsetTimesSDK); is {
			client = live
