...
class SunsetTimesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SunsetTimesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SunsetTimesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SunsetTimesTestRunner.getenv("SUNSET_TIMES_TEST_LIVE")
        override = SunsetTimesTestRunner.getenv("SUNSET_TIMES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SunsetTimesTestRunner.getenv(key)
        ...
        explain = SunsetTimesTestRunner.getenv("SUNSET_TIMES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SUNSET_TIMES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SunsetTimesTestRunner._test_control is not None:
            return SunsetTimesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SunsetTimesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SunsetTimesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SunsetTimesTestRunner.load_test_control()
...
def load_env_local():
    SunsetTimesTestRunner.load_env_local()


def env_override(m):
    return SunsetTimesTestRunner.env_override(m)


def entity_data(v):
    return SunsetTimesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SunsetTimesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SunsetTimesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SunsetTimesTestRunner.load_test_control()


def live_client_options():
    return SunsetTimesTestRunner.live_client_options()


def live_delay_ms():
    return SunsetTimesTestRunner.live_delay_ms()
