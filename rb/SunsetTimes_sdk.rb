...
require_relative 'SunsetTimes_types'
...
class SunsetTimesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SunsetTimesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SunsetTimesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SunsetTimesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SunsetTimesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SunsetTimesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SunsetTimesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SunsetTimesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = SunsetTimesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SunsetTimesError.new(
        "graphql_error", "SunsetTimesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SunsetTimesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
