...
class SunriseAndSunsetEntity
  ...
  def make
    opts = @_entopts.dup
    SunriseAndSunsetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SunsetTimesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [SunriseAndSunset, Hash] the current SunriseAndSunset data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SunsetTimesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of SunriseAndSunset fields)
  def match_get
  ...
  #
  # @param reqmatch [SunriseAndSunsetLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.SunriseAndSunset.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [SunriseAndSunset, Hash] the loaded SunriseAndSunset; raises SunsetTimesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SunsetTimesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
