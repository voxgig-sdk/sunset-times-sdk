...
module SunsetTimesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SunsetTimesResponse.new({})
    ...
    if fetch_err
      ...
      response = SunsetTimesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SunsetTimesResponse.new(fetched)
    else
