...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.SUNSETTIMES_TEST_LIVE ||
    'TRUE' === process.env.SUNSETTIMES_TEST_OVERRIDE
  ) {
  ...
  m.SUNSETTIMES_TEST_EXPLAIN = process.env.SUNSETTIMES_TEST_EXPLAIN || m.SUNSETTIMES_TEST_EXPLAIN
