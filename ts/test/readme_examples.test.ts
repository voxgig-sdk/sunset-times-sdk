...
//     (\`new SunsetTimesSDK(...)\` / \`SunsetTimesSDK.test(...)\` become
//     \`SunsetTimesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"sunrise_and_sunset":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SunsetTimesSDK(...)\` and any
// \`SunsetTimesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SunsetTimesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SunsetTimesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
