...
local SunriseAndSunsetEntity = {}
SunriseAndSunsetEntity.__index = SunriseAndSunsetEntity


function SunriseAndSunsetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SunriseAndSunsetEntity)
  self._name = "sunrise_and_sunset"
  self._client = client
...
function SunriseAndSunsetEntity:get_name()
  return self._name
end


function SunriseAndSunsetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SunriseAndSunsetEntity.new(self._client, opts)
end
...
function SunriseAndSunsetEntity:mark_deleted()
  self._deleted = true
end


function SunriseAndSunsetEntity:deleted()
  return true == self._deleted
end


function SunriseAndSunsetEntity:data_set(args)
  if args ~= nil then
...
function SunriseAndSunsetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SunriseAndSunsetEntity:match_set(args)
  if args ~= nil then
...
function SunriseAndSunsetEntity:match_get()
...
function SunriseAndSunsetEntity:stream(action, args, callopts)
...
---@param reqmatch SunriseAndSunsetLoadMatch
---@param ctrl? table
---@return SunriseAndSunset
---@return string? err
function SunriseAndSunsetEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SunriseAndSunsetEntity:_run_op(ctx, post_done)
  local utility = self._utility
