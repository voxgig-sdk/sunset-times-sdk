...
[MCP](https://modelcontextprotocol.io) server exposing the SunsetTimes SDK as
two agent tools — `sunset-times_list` and `sunset-times_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sunset-times-mcp)
make build
...
claude mcp add --scope user sunset-times \
  -- /absolute/path/to/sunset-times-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sunset-times-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sunset-times_list: first page of records
{ "entity": "sunrise_and_sunset" }
{ "entity": "sunrise_and_sunset", "query": { } }

// sunset-times_load: one record by id
{ "entity": "sunrise_and_sunset", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sunset-times-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sunset-times \
     -- "$PWD"/dist/*/sunset-times-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sunset-times_list` and `sunset-times_load` tools now appear
   in new sessions. Ask the agent to *"list sunrise_and_sunset using sunset-times"*
   and it calls `sunset-times_list` with `{"entity":"sunrise_and_sunset"}`.
...
```sh
export SUNSET_TIMES_APIKEY=sk_live_xxx            # API key
export SUNSET_TIMES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `sunset-times_list` tool
...
### Call the `sunset-times_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `sunset-times_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sunset-times_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SUNSET_TIMES_APIKEY` | API key sent with every request. |
| `SUNSET_TIMES_BASE` | Optional override of the API base URL. |
...
```sh
./sunset-times-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sunset-times_load","arguments":{"entity":"sunrise_and_sunset","query":{"id":1}}}}'
```
