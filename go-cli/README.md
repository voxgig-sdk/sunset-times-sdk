...
boru-driven command-line client **and** interactive REPL for the SunsetTimes
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sunset-times-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/sunset-times-cli load 1 sunrise_and_sunset
   ```
...
```sh
./sunset-times-cli load 1 sunrise_and_sunset          # scalar shorthand for {id:1}
./sunset-times-cli load '{id:1}' sunrise_and_sunset     # explicit match map
```
...
./sunset-times-cli load 1 sunrise_and_sunset
```
...
Run with no arguments to open a REPL (prompt `sunset-times>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `sunrise_and_sunset`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sunset-times-cli`. |
...
(`./sunset-times-cli <expr>`) and interactively (the REPL), and expressions compose the
